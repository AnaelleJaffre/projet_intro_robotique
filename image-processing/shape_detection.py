...
def center_of_zone(img, h_start, h_end):
    ...
    z_start = (np.argmin(img[h_start]), len(img) - 1 - np.argmin(img[h_start][::-1]))
    print(z_start)
    z_start_center = (z_start[1] - z_start[0]) / 2
    z_end = (np.argmin(img[h_end]), len(img) - 1 - np.argmin(img[h_end][::-1]))
    z_end_center = (z_end[1] - z_start[0]) / 2
